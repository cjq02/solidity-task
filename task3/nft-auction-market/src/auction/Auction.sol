...
contract Auction is IAuction, Ownable, ReentrancyGuard {
    ...
    mapping(address => mapping(address => uint256))
        public pendingTokenWithdrawals;
    ...
    modifier onlyActiveAuction(uint256 auctionId) {
        require(
            auctions[auctionId].status == IAuction.AuctionStatus.Active,
            "Auction not active"
        );
        require(
    ...
    function placeBid(
        uint256 auctionId
    ) external payable onlyActiveAuction(auctionId) nonReentrant {
        ...
        if (bids[auctionId].length > 0) {
            IAuction.Bid memory currentHighest = bids[auctionId][
                highestBidIndex[auctionId]
            ];
            ...
            // 记录待退还金额（Pull 模式）
            pendingETHWithdrawals[currentHighest.bidder] += currentHighest
                .amount;
        }
    ...
    function placeBidWithToken(
        ...
        if (bids[auctionId].length > 0) {
            ...
            // ========== Effects ==========
            // 记录待退还金额（Pull 模式）
            pendingTokenWithdrawals[currentHighest.bidder][
    ...
    function endAuction(uint256 auctionId) external nonReentrant {
        ...
        IAuction.Bid memory winningBid = bids[auctionId][
            highestBidIndex[auctionId]
        ];
    ...
    function getAuction(
        uint256 auctionId
    ) external view returns (IAuction.AuctionInfo memory) {
    ...
    function getHighestBid(
        uint256 auctionId
    ) external view returns (IAuction.Bid memory) {
        if (bids[auctionId].length == 0) {
            return
                Bid({
                    bidder: address(0),
